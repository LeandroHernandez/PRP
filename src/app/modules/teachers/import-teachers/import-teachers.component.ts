...
export class ImportTeachersComponent implements OnInit {
    ...
    private async getTeacherByEmail(teacherCSV: Teacher) {
        ...
        const response = await this
            .teacherService
            .findTeacherEmail(teacherCSV.teacher_email);
        response
            .subscribe(teacherResponse => {
                const teacher = teacherResponse as Array<Teacher>
                if (teacher.length > 0) {
                    teacherSave = teacher[0];
                    if (teacherSave.teacher_unit_educational.includes(this.infoUser.unit_educational_id)) {
                        console.log('NO INGRESAR')
                    } else {
                        teacherSave.teacher_unit_educational.push(this.infoUser.unit_educational_id);
                        this.saveTeacher(teacherSave, false);
                    }
                } else {
                    teacherSave = teacherCSV;
                    teacherSave.teacher_id = new Date().getTime().toString();
                    teacherSave.teacher_unit_educational = [this.infoUser.unit_educational_id];
    ...
    private async saveTeacher(teacher: Teacher, isNew: boolean) {
        console.log(teacher)
        this.teacherService
            .addUnitEducational(teacher, isNew)
            .then(r => '');
