...
export class AcademyareaComponent implements OnInit {
  ...
  constructor(private academyareaService: AcademyareaService,
  ...
  ngOnInit(): void {
    ...
    this.dataTable = {
      headerRow: ['ID', 'NIVEL', 'SUBNIVEL', 'CODIGO', 'NOMBRE', 'ESTADO', 'ACCIONES'],
      footerRow: ['ID', 'NIVEL', 'SUBNIVEL', 'CODIGO', 'NOMBRE', 'ESTADO', 'ACCIONES'],
      dataRows: []
    };
    ...
    levels.forEach(async (level: Level) => {
      await this.sublevelService.getALLSublevelActive(level.level_id).subscribe(allSublevels => {
        allSublevels.forEach((subLevel) => {
          this.academyareaService.getAllAcademyArea(level.level_id, subLevel.sublevel_id).subscribe(areas => {
            areas.forEach(area => {
              area.level_name = level.level_name;
              area.sublevel_name = subLevel.sublevel_name
              if (this.isEdit === true && this.arrayAreas.find(data => data.academyarea_id === area.academyarea_id)) {
                const i = this.arrayAreas.findIndex(data => data.academyarea_id === area.academyarea_id)
                this.arrayAreas.splice(i, 1, area)
              }
              if (this.arrayAreas.find(data => data.academyarea_id === area.academyarea_id)) {
