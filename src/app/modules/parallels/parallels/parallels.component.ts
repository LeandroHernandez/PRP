...
export class ParallelsComponent implements OnInit {
  ...
  public async onChangeSublevel(sublevelId): Promise<void> {
    await this.gradeService
      .getAllGradesActive(this.levelId, sublevelId)
      .subscribe((grades) => {
      ...
      await this.sublevelService
        ...
        .subscribe((allSublevels) => {
          allSublevels.forEach((subLevel) => {
            this.gradeService
              .getAllGradesActive(level.level_id, subLevel.sublevel_id)
              .subscribe((dataGrade) => {
                dataGrade.forEach((grade) => {
                  this.parallelService
                    .getAllParallels(
                      level.level_id,
                    ...
                    .subscribe((parallels) => {
                      parallels.forEach((parallel) => {
                        ...
                        if (
                          ...
                          const i = this.arrayParallels.findIndex(
                            (data) => data.parallel_id === parallel.parallel_id
                          );
