...
export class GradesComponent implements OnInit {
    ...
    levels.forEach(async (level: Level) => {
      await this.sublevelService.getALLSublevelActive(level.level_id).subscribe(allSublevels => {
        allSublevels.forEach((subLevel) => {
          this.gradeService.getAllGrades(level.level_id, subLevel.sublevel_id).subscribe(dataGrade => {
            dataGrade.forEach(grade => {
              grade.level_name = level.level_name;
              grade.sublevel_name = subLevel.sublevel_name
              if (this.isEdit === true && this.arrayGrades.find(data => data.grade_id === grade.grade_id)) {
                const i = this.arrayGrades.findIndex(data => data.grade_id === grade.grade_id)
  ...
  */
  ...
  /** Función Guardar*/
  public async saveSchoolGrade(grade: SchoolGrade, isValid: boolean): Promise<void> {
    if (isValid) {
