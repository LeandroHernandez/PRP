...
import { DataTable } from "../../../models/interfaces/data-table";
import swal from "sweetalert2";
import { TeacherSubjectsOriginalService } from "app/services/teacher-subjects/teacher-subjects-original.service";
...
export class ShowPlanificationComponent implements OnInit {
  ...
  constructor(
    ...
    public ts: TeacherSubjectsOriginalService,
    private adminEducationalUnitService: AdminEducationalUnitService,
    private teacherService: TeachersService,
    private fb: FormBuilder,
    public carga: LoadDataService,
  ...
  ngOnInit(): void {
    ...
    this.tableResourceEssays = $("#datatablesResourceEssays").DataTable({});
    this.tableResourceEvaluations = $(
      "#datatablesResourceEvaluations"
    ).DataTable({});
    this.activeClass = {};
    this.editButton = false;
    this.editRow = 0;
  ...
  getSubjectUnits(teacher, subject) {
    this.units = [];
    this.ts
      .getUnitsNoRealTimeQuery(teacher, subject)
      .then((doc) => {
        if (!doc.empty) {
          doc.forEach((x) => {
            this.units.push(x.data());
  ...
  getUnitResources(unit, subject) {
    this.arraySubjectTeachers.forEach((teacher) => {
      if (unit !== undefined) {
        this.ts
          .getUnitResources(teacher, unit, subject)
          .subscribe((unitResouce) => {
            this.unitResources.text = unitResouce;
          });
        this.ts
          .getExternalFiles(this.activeSubject)
          .pipe(
            map((resource: any) =>
              resource.filter(
                (r) =>
                  r.id_grade === this.activeSubject.grade_id &&
  ...
  getExternalResources() {
    ...
    this.ts
      ...
      .pipe(
        map((resource: any) =>
          resource.filter((r) => r.id_grade === this.activeSubject.grade_id)
        )
      )
      .subscribe((files) => {
        this.externalResources.files = files;
    ...
    this.arraySubjectTeachers.forEach((teacher) => {
      this.ts
        .getPractices(teacher.teacher_unit_educational[0])
        .pipe(
          map((practice: Adminpractices[]) =>
            practice.filter(
              (p) =>
                p.subject_id === this.activeSubject.subject_id &&
                p.grade_id === this.activeSubject.grade_id &&
      ...
      this.ts
        .getEssays(teacher.teacher_unit_educational[0])
        .pipe(
          map((essays: Adminpractices[]) =>
            essays.filter(
              (e) =>
                e.subject_id === this.activeSubject.subject_id &&
                e.grade_id === this.activeSubject.grade_id &&
      ...
      this.ts
        .getEvaluationsFromUnitEducationalId(
          teacher.teacher_unit_educational[0]
        )
        .pipe(
          map((evaluations: Adminpractices[]) =>
            evaluations.filter(
              (e) =>
                e.subject_id === this.activeSubject.subject_id &&
                e.grade_id === this.activeSubject.grade_id &&
  ...
  public getTeachersFromUnitEducational() {
    ...
    this.adminEducationalUnitService
      .getTeachersFromUnitEducational(this.userInfo.unit_educational_id)
      .pipe(
        map((t: Teacher[]) =>
  ...
  public getSubjectTeachers(subject: Subject) {
    this.arrayTeachers.forEach((teacher) => {
      ...
      this.subjectFromTeacherSubscription = this.adminEducationalUnitService
        .getSubjectTeachers(
          teacher.teacher_id,
          this.userInfo.unit_educational_id,
          this.activeSubject.parallel_id,
          subject.subject_id
        )
        .subscribe((dataSubjectTeacher) => {
          this.arraySubjectTeachers = [];
  ...
  getClassFromUnit(unit, subject) {
    ...
    this.arraySubjectTeachers.forEach((teacher) => {
      if (unit !== undefined) {
        this.ts.getClassFromUnit(teacher, unit, subject).subscribe((resp) => {
          this.classes = resp;
          this.classes.forEach(async (item) => {
            await this.ts
              .getClassResources(
