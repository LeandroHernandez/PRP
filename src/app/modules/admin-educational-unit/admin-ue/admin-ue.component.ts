...
export class AdminUEComponent implements OnInit, OnDestroy {
  ...
  // public getGradesByEducationalUnit() {
  //   this.gradesSubscription = this.adminEducationalUnitService
  //     .getGradesFromUnitEducational(this.infoUser.unit_educational_id)
  //     .subscribe((dataGrades) => (this.arrayGrades = dataGrades));
  // }
  public getGrades() {
    console.log("true");
    this.gradesSubscription = this.adminEducationalUnitService
      .getGrades()
      .subscribe((dataGrades) => (this.arrayGrades = dataGrades));
    console.log({ arrayGrades: this.arrayGrades });
  }

  public selectGrade(grade) {
    this.getParallelsContainingStudents(grade);
    this.grade_id = grade;
    this.parallelsSubscription = this.adminEducationalUnitService
      .getParallelsFromUnitEducational(grade, this.infoUser.unit_educational_id)
      .pipe(
        map((p: Parallels[]) =>
          p.filter((parallel) => parallel.grade_id === grade)
        )
      )
      .subscribe((data) => (this.arrayParallels = data));
  }

  public getParallelsContainingStudents(grade) {
    this.arrayParallelsActive = [];
    this.studentSusbcription = this.adminEducationalUnitService
      .getParallelsFromStudent(this.infoUser.unit_educational_id)
      .pipe(
        map((student: Student[]) =>
          student.filter(
            (s) => s.student_grade_id === grade && s.student_parallel_id !== ""
          )
        )
      )
      .subscribe((dataStudent) => {
        ...
        this.arrayStudent.forEach((student) => {
          if (student.student_parallel_id) {
            this.parallelsSubscription = this.adminEducationalUnitService
              .getParallelByParallelId(
                student.student_parallel_id,
                this.infoUser.unit_educational_id
              )
              .subscribe((dataP) => {
                if (
                  this.arrayParallelsActive.find(
                    (data) => data.parallel_id === student.student_parallel_id
                  )
                ) {
                  return;
                } else {
