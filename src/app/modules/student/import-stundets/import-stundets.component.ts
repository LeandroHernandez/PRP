...
export class ImportStundetsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dataTable = {
      headerRow: ['IDENTIFICACIÓN', 'NOMBRE', 'APELLIDO', 'EMAIL', 'TELEFONO', 'IDGRADO', 'IDPARALELO', 'ESTADO'],
      footerRow: ['IDENTIFICACIÓN', 'NOMBRE', 'APELLIDO', 'EMAIL', 'TELEFONO', 'IDGRADO', 'IDPARALELO', 'ESTADO'],
      dataRows: []
    };
  }
  ...
  private async validateStudent(teacherCSV: Student) {
    ...
    const responseEmail = await this.studentService.finStudentByEmail(teacherCSV.student_email).pipe(take(1)).toPromise();
    const responseIdentification = await this.studentService.finStudentByIdentification(teacherCSV.student_identification).pipe(take(1)).toPromise();
    if (responseEmail.length === 0 && responseIdentification.length === 0) {
      ...
      await this.studentService.finStudentByEmail(teacherCSV.student_email).subscribe(respuesta => {
        if (respuesta.length !== 0) {
          this.sendEmailService.sendEmailConfirmImportStudent(respuesta[0], teacherCSV.student_email)
        } else {
          this.studentService.finStudentByIdentification(teacherCSV.student_identification).subscribe( res => {
            if (res.length !== 0) {
              this.sendEmailService.sendEmailConfirmImportStudent(res[0], teacherCSV.student_email)
            }
          });
        }
      });
