...
export class ListstudentComponent implements OnInit {
  ...
  async showDataRepresentative(student: Student) {
    ...
    const response = await this.representativeService.getrepresentative(
      student.student_representant
    );
  ...
  async getStudentData() {
    ...
    const response = await this.studentService.allstudent();
    response.subscribe((StudentD) => {
      aux = [];
      StudentD.forEach((element) => {
        if (
          element.student_status == null ||
          element.student_status ||
          element.student_enabled == null ||
          element.student_enabled
        ) {
          aux.push(element);
        }
      });
  ...
  async saveStudent(StudentD: Studentdocum, isValid: boolean, isEdit: boolean) {
    if (isValid) {
      if (isEdit) {
        // await this.studentService.updateStudents(
        //   StudentD,
        //   this.infoUser.unit_educational_id
        // );
        await this.studentService.updateStudents(StudentD);
        ...
        if (this.checkID(this.data_student, StudentD.student_identification)) {
          ...
          await this.studentService.saveStudents(StudentD).then(() => {
            $("#exampleModalCenter").modal("hide");
            swal({
              title: "Ok",
  ...
  public blackStudent(StudentD: Studentdocum, studentEnable: boolean) {
    this.studentService.blockStudent(
      StudentD.student_id,
      studentEnable,
      StudentD.student_email
    );
  }
  ...
  checkID(array: Studentdocum[], identification: string) {
    ...
    for (let i = 0; i < array.length; i++) {
      if (
        array[i].student_identification.replace(" ", "") ===
