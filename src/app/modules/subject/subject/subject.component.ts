...
export class SubjectComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dataTable = {
      headerRow: ['ID', 'NIVEL', 'SUBNIVEL', 'AREA ACADEMICA', 'CODIGO', 'ASIGNATURA', 'COLOR', 'ESTADO', 'ACCIONES'],
      footerRow: ['ID', 'NIVEL', 'SUBNIVEL', 'AREA ACADEMICA', 'CODIGO', 'ASIGNATURA', 'COLOR', 'ESTADO', 'ACCIONES'],
      dataRows: []
    };
 ...
 */
  public async onChangeSublevel(sublevelId): Promise<void> {
    const dataAreaAcademy = await this.areaAcademyService.getAllAcademyAreaActives(this.levelId, sublevelId);
    ...
    levels.forEach(async (level: Level) => {
      await this.sublevelService.getALLSublevelActive(level.level_id).subscribe(allSublevels => {
        allSublevels.forEach((subLevel) => {
          this.areaAcademyService.getAllAcademyAreaActives(level.level_id, subLevel.sublevel_id).subscribe(areas => {
            areas.forEach(area => {
              this.subjectService.getAllSubjects(level.level_id, subLevel.sublevel_id, area.academyarea_id).subscribe(subjects => {
                subjects.forEach(subject => {
                  ...
                  if (this.isEdit === true && this.arraySubjects.find(data => data.subject_id === subject.subject_id)) {
                    const i = this.arraySubjects.findIndex(data => data.subject_id === subject.subject_id)
