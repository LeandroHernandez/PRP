...
export class ViewStatisticsStudentComponent implements OnInit, OnDestroy {
  @ViewChild("datatablesEvaluation", { static: false })
  datatablesEvaluation: ElementRef;
  @ViewChild("datatablesPractice", { static: false })
  datatablesPractice: ElementRef;
  ...
  ngOnInit(): void {
    ...
    this.academicPeriodAcademic = JSON.parse(
  ...
  exportToExcel() {
    const ws: xlsx.WorkSheet = xlsx.utils.table_to_sheet(
      this.datatablesEvaluation.nativeElement
    );
  ...
  exportToExcelPractices() {
    const ws: xlsx.WorkSheet = xlsx.utils.table_to_sheet(
      this.datatablesPractice.nativeElement
    );
    const wb: xlsx.WorkBook = xlsx.utils.book_new();
    xlsx.utils.book_append_sheet(wb, ws, "Modulares");
  ...
  async getSubjectUnits() {
    const subjects = await this.ts.getUnitsNoRealTimeQuery(
      this.infoUser,
      this.subjectSelect,
  ...
  private async getClassFromUnit(unit, subject) {
    this.arrayClasses = [];
    this.arrayClasses = await this.ts
      .getClassFromUnitTrue(
        this.infoUser,
        unit,
        subject,
        this.academicPeriodAcademic[0].academic_year_name
      ...
      .pipe(
        take(1),
        map((clase) => clase.filter((c) => c.class_status === true))
      )
      .toPromise();
  ...
  public async getStudentsFromGrade() {
    this.arrayStudents = await this.adminEducationalService
      .getStudentsAssignedToGrade(this.infoUser.teacher_unit_educational[0])
      .pipe(take(1))
      .toPromise();
    this.arrayStudentsFromGrade = this.arrayStudents.filter(
      (student) =>
        student.student_grade_id === this.subjectSelect.grade_id &&
        student.student_parallel_id === this.subjectSelect.parallel_id
    );
    this.arrayStudentsFromGrade.sort((a, b) =>
      a.student_lastname > b.student_lastname
        ? 1
        : b.student_lastname > a.student_lastname
        ? -1
        : 0
    );
  }
  ...
  private getPreviousClass() {
    this.ts
      .getPreviousClass(
        ...
        this.activeUnit,
        this.subjectSelect,
        this.arrayClasses[this.arrayClasses.length - 1].class_id,
        this.academicPeriodAcademic[0].academic_year_name
      )
      .subscribe((data) => {
        if (data.length > 0) {
        ...
        this.arrayClasses.forEach((unitClass, i) => {
          if (unitClass.class_evaluation !== undefined) {
            if (unitClass.class_evaluation.length > 0 && count === 1) {
              if (
                unitClass.class_resources.find((data) => data.evaluation_name)
              ) {
                return;
              } else {
          ...
          this.resourcesSubscription = this.ts
            .getStudentResources(
              student.student_id,
                ...
                .subscribe((evaluation: any) => {
                  evaluation.forEach((element) => {
                    if (element !== undefined) {
                      if (
                        element.evaluationStatus === true ||
                        element.evaluationId === undefined
                      ) {
                  ...
                  if (
                    student.student_classes[i] !== undefined &&
                    unitClass.class_resources !== undefined
                  ) {
                    student.student_classes[i].class_resources =
                      unitClass.class_resources;
                    ...
                    resources.forEach((r: any) => {
                      ...
                      student.student_classes[i].student_total_time =
                        r.time_spend;
                      if (r.totalScore !== undefined) {
                        r.intents = r.intents;
                      }
                      student.student_classes[i].student_total_attempts +=
                        r.intents;
                      if (
                        r.resource_status === true ||
                        r.totalScore !== undefined
                      ) {
                        student.student_classes[
                          i
                        ].student_total_resources_in_true += 1;
                        if (resources.length !== undefined) {
                          ...
                          const prueba =
                            student.student_classes[i]
                              .student_total_resources_in_true /
                            student.student_classes[i].class_resources.length;
                          student.student_classes[
                            i
                          ].student_porcentage_resources = Math.floor(
                            prueba * 100
                          );
                        }
                      }
  ...
  async showDetail(student_id: string, subject_id: string, unit_id: string) {
    ...
    this.studentSelect = await this.studentService
  ...
  private async findResources() {
    if (this.arrayClasses.length > 0) {
      const responseResources = await this.studentService.getClass(
        this.student_id,
        this.subject_id,
        this.unit_id,
        this.activeClass.class_id
      );
      responseResources.subscribe((resourcesResponseList) => {
        ...
        this.ts
          ...
          .pipe(take(1))
          .subscribe((evaluations: EvaluationSummary[]) => {
        ...
        if (practiceAttempts != null && practiceAttempts.length > 0) {
          this.adminEvaluationService
            .getpracticeAttempts(
              ...
              this.activeClass.class_id,
              resource,
              practiceAttempts[0].payload.doc.id
            )
            .subscribe((attemptsDetaills) => {
  ...
  public newAttempEvaluation(evaluation) {
    ...
    swal({
      title:
        "¿Confirma que desea activar nuevamente la evaluación al estudiante?",
      type: "warning",
      ...
      if (result.value) {
        swal(
          "Ok!",
          "Se ha activado nuevamente la evaluación para el estudiante.",
          "success"
        );
        this.adminEvaluationService.activeEvaluation(
          this.student_id,
          this.subject_id,
          this.unit_id,
          this.activeClass.class_id,
  ...
  public async showTryDetail(resource: Resource) {
    if (resource.evaluationData === undefined) {
      const responseEntry = await this.studentService.getEntrySnapshot(
        this.student_id,
        this.subject_id,
        this.unit_id,
        this.activeClass.class_id,
      ...
      responseEntry.subscribe((entryResponseList) => {
  ...
  private forumEnabledList(classList, unitId: string) {
    for (const classObject of classList) {
      ...
      this.forumService
        .findForumClass(
          this.infoUser.teacher_unit_educational[0],
        ...
        .subscribe((forum) => {
          ...
          this.enabledListForum.push({
            codeStringState: codeString,
            resultForum: forumDoc,
          });
  ...
  public showDataForum(classId: string, unitId: string) {
    ...
    this.forumService
      ...
      .subscribe((questionList) => {
        this.forumQuestionDocumentList = this.sortListquestion(
          questionList as Array<ForumQuestionDTO>
        );
  ...
  private getCodeString(classId: string, unitId: string) {
    return (
      this.infoUser.teacher_unit_educational +
      ";" +
      this.subjectSelect.parallel_id +
      ";" +
      this.subjectSelect.subject_id +
  ...
  public saveContribution(
    forumQuestionDocument: ForumQuestionDocument,
    index: number
  ) {
    this.documentForumAnswerSave.answer_id = String(
    ...
    this.documentForumAnswerSave.answer_user_name =
      this.infoUser.teacher_name + " " + this.infoUser.teacher_surname;
    ...
    this.forumService.saveContribution(
      this.infoUser.teacher_unit_educational[0],
      this.subjectSelect.parallel_id,
      this.subjectSelect.subject_id,
      this.unitIdSelected,
  ...
  public validateExistence(questionCOde: string): boolean {
    for (const x of this.forumQuestionDocumentList) {
      if (x.question_id === questionCOde) {
        return (
          (x.studentAnswersList as string[]).indexOf(
            this.infoUser.teacher_id
          ) !== -1
        );
      }
    }
      ...
      .subscribe((answersLIst) => {
        ...
        for (const forumQuestion of this.forumQuestionDocumentList) {
          if (forumQuestion.question_id === forumQuestionDocument.question_id) {
            this.forumQuestionDocumentList[cont].documentForumAnswerList =
              new Array<DocumentForumAnswer>();
            this.forumQuestionDocumentList[cont].documentForumAnswerList =
              this.sortList(answersLIst as Array<DocumentForumAnswer>);
            let maxSize = 0;
            this.forumQuestionDocumentList[cont].documentForumBestAnswer =
              new DocumentForumAnswer();
            for (const x of this.forumQuestionDocumentList[cont]
              .documentForumAnswerList) {
  ...
  public likeAnswer(
    ...
    this.forumService.addLikeContribution(
      this.infoUser.teacher_unit_educational[0],
      this.subjectSelect.parallel_id,
      this.subjectSelect.subject_id,
      this.unitIdSelected,
  ...
  public getUserLikedAnswer(
    documentForumAnswerList: DocumentForumAnswer
  ): boolean {
    return (
      documentForumAnswerList.answer_count_like.indexOf(
        this.infoUser.teacher_id
      ) !== -1
    );
  }
  ...
  saveNewQuestion() {
    if (this.forumQuestionDocumentSave !== null) {
      const date = new Date();
      this.forumQuestionDocumentSave.question_id = new Date()
        .getTime()
        .toString();
      ...
      this.forumService
        .createForumQuestionStudent(
          this.infoUser.teacher_unit_educational[0],
          this.subjectSelect.parallel_id,
          this.subjectSelect.subject_id,
          this.activeUnit.unit_id,
          this.classIdSelected,
          this.forumQuestionDocumentSave
        )
        .then((r) => {
          this.forumQuestionDocumentSave = new ForumQuestionDocument();
        });
  ...
  private getDate(date: Date) {
    return (
      date.getFullYear() +
      "/" +
      (date.getMonth() + 1 > 12 ? 12 : date.getMonth() + 1) +
      "/" +
      date.getDate()
    );
  }
    ...
    this.forumService
      ...
      .subscribe((questionList) => {
        ...
        for (const forumQuestion of questionList as Array<ForumQuestionDocument>) {
          if (
            forumQuestion.studentAnswersList.indexOf(
              this.infoUser.teacher_id
            ) === -1
          ) {
            cont = cont + 1;
