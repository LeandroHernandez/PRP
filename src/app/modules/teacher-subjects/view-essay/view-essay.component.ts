...
import {
  ...
  RubricJudgementItem,
  RubricPoint,
} from "../../../models/class/class.documentrubric";
...
export class ViewEssayComponent implements OnInit {
  ...
  public async viewEssay(essay_id: String) {
    this.essay = await this.adminEssayService
      .getEssay(essay_id)
      .pipe(take(1))
      .toPromise();
  ...
  private async getOptionsFromEssay(essay: String) {
    this.allOptionsFromEssay = await this.adminEssayService
      .getOptionsFromEssay(essay)
      .pipe(take(1))
      .toPromise();
  ...
  public getStudentAnswer() {
    const studentAnswer = this.adminEssayService.getStudentAnswerV2(
      this.student.student_id,
      this.subjectId,
      this.unitId,
      this.classId,
      this.essayId.resource_id
    );
  ...
  public getRubric() {
    if (this.essayId.resource_id_original) {
      this.adminEssayService
        .getRubric(this.essayId.resource_id_original)
        .subscribe((resp) => {
          if (resp.length > 0) {
            this.hasRubric = true;
            this.essayRubric = new Rubric(resp[0]);
            this.getRubricPoints(
              this.essayId.resource_id_original,
              this.essayRubric
            );
            this.getStudentCalification().then(
              (items: RubricJudgementItem[]) => {
                if (items.length > 0) {
  ...
  private getRubricPoints(essay, rubric) {
    this.adminEssayService
      .getRubricPoints(essay, rubric)
      .subscribe((resp: RubricPoint[]) => {
  ...
  private getRubricJudgements(essay, rubric, studentScoreList?: any) {
    this.adminEssayService
      .getRubricJudgements(essay, rubric)
      .subscribe((resp: RubricJudgement[]) => {
        this.rubricJudgements = resp;
        this.rubricJudgements.forEach((r) => {
          this.adminEssayService
            .getRubricJudgementItems(essay, rubric, r)
            .subscribe((items: RubricJudgementItem[]) => {
              studentScoreList.forEach((score) => {
                items.some((it: any) => {
                  if (
                    it.rubric_judgement_item_id ===
                    score.rubric_judgement_item_id
                  ) {
                    it.selected = true;
                  }
                });
  ...
  async saveComment() {
    await this.adminEssayService
      .saveComment(
        ...
        this.classId,
        this.essayId.resource_id,
        this.essayAnswer.resource_comment
      )
      .then(() => {
        swal({
          title: "Ok",
  ...
  async saveQualification(isNew) {
    this.isSave = !isNew;
    this.adminEssayService.saveStudentQualification(
      this.student.student_id,
      this.subjectId,
      this.unitId,
      this.classId,
  ...
  itemIsEnabled(
    ...
    return !!rubric_judgement_items.find(
      (value) =>
        value.rubric_judgement_item_id === item.rubric_judgement_item_id &&
        value.selected === true
    );
  }
}
