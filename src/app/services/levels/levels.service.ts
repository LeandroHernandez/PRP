...
export class LevelsService {
    ...
    constructor(private db: AngularFirestore) {
        this.levelCollection = this.db.collection('config').doc('general_config').collection<Level>(`levels`)
        this.levelUnitEducationalCollection = this.db.collection('config').doc('general_config').collection<Level>(`levels`)
    }
    ...
    public getLevelsUnitEducational(unitEducational: UnitEducational, level: Level): Observable<any> {
    ...
    public async getLevelsUnitEducationalNotRealTime(unitEducational: UnitEducational): Promise<QuerySnapshot<DocumentData>> {
        return await this.db
            .collection(unitEducational.unit_educational_city.toLowerCase())
            .doc(unitEducational.unit_educational_id.toString())
            .collection<Level>('levels', ref => ref.where('level_status', '==', true))
            .get().toPromise();
    }
     ...
     */
    public getSubLevelsUnitEducational(unitEducational: UnitEducational, sublevel: SubLevels): Observable<any> {
        const collection = this.db
            .collection(unitEducational.unit_educational_city.toLowerCase())
            .doc(unitEducational.unit_educational_id.toString())
            .collection<Level>(`sublevels`).doc(`${sublevel.sublevel_id}`);
        return collection.valueChanges();
    }

    public async getSubLevelsUnitEducationalNotRealTime(unitEducational: UnitEducational): Promise<QuerySnapshot<DocumentData>> {
        return await this.db
            .collection(unitEducational.unit_educational_city.toLowerCase())
            .doc(unitEducational.unit_educational_id.toString())
            .collection<Level>('sublevels', ref => ref.where('sublevel_status', '==', true))
            .get().toPromise();
    }
    ...
    public getLevel(level_id: string): Observable<any> {
        return this.db.collection('config').doc('general_config').collection<Level>(`levels`).doc(`${level_id}`).valueChanges();
    }
