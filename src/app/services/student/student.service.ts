...
export class StudentService {
  ...
  public finStudentByIdentification(identificationStudent: String): Observable<any> {
    return this
        .db
        .collection('student',
                ref => ref.where(
  ...
  public getClass(studentId: string, subjectId: string,
                  unitId: string, classId: string): Observable<any> {
    return this.db
        .collection('student')
        .doc(studentId)
        .collection('subjects')
  ...
  public getEntry(studentId: string, subjectId: string,
                  unitId: string, classId: string,
  ...
  public getEntrySnapshot(studentId: string, subjectId: string,
    unitId: string, classId: string,
    resourceID: string): Observable<any> {
return this.db
.collection('student')
...
.snapshotChanges()
  ...
  public getPracticeDetail(studentId: string, subjectId: string,
    unitId: string, classId: string,
    resourceID: string) {
    return this.db
      .collection('student')
  ...
  public getEntryAttemptDetail(studentId: string, subjectId: string,
    unitId: string, classId: string,
    resourceID: string, id_entry: String, practiceId: String) {
    return this.db
      .collection('student')
      .doc(studentId)
      .collection('subjects')
      ...
      .doc(resourceID)
      .collection('entry')
      .doc(`${id_entry}`).collection('practiceAttempts')      .snapshotChanges().pipe(map(actions => {
        return actions.map(a => {
          const data = a.payload.doc.data() as Adminpractices;
          const id = a.payload.doc.id;
          return { id, ...data };
  ...
  public getDetailEntry(studentId: string, subjectId: string,
    unitId: string, classId: string,
    resourceID: string, id_entry: String, practiceId: String) {
    return this.db
      .collection('student')
      ...
      .doc(resourceID)
      .collection('entry')
      .doc(`${id_entry}`).collection('practiceAttempts').doc(`${practiceId}`).collection('attemptDetail').valueChanges()

  }

  public blockStudent(studentID: string, student_enabled: boolean, studentEmail: string) {
    const student = this.db.collection('student')
        .doc(studentID);
    student.update({
